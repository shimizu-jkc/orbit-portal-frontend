...
AWSを利用するにはまず、[AWSアカウントを作成](https://aws.amazon.com/jp/register-flow/)する必要があります。
AWSアカウントとは、テナントのようなもので、権限・リソース・請求を明確に分離する単位であり、プロジェクトや利用目的ごとに作られるのが一般的です。
AWSアカウントの作成には、連絡先となるEメールアドレスの登録や、クレジットカードによる支払い情報の入力を行った後、ユーザーの
...
## 利用するAWSサービス
OrBITでは、AWSアカウントを組織的に管理するため、以下のサービスを利用します。

- ***AWS Organizations***

    複数のAWSアカウントを組織として一元的に管理するサービスです。
    AWSアカウントを目的やドメイン別に階層的に管理することができ、組織内のセキュリティガバナンスや一括請求による仕組みを実現することができます。
...
また、1つのプロジェクトで利用目的別に複数のプロジェクトアカウントを持つこともできます。
...
| 検証環境 | poc | 技術調査や概念実証を行うための環境 |

::: danger IMPORTANT
本番環境の払い出しに関しては、OrBIT運用チームによる[審査](#本番環境の払い出しに関する審査)があります。
:::

### 本番環境の払い出しに関する審査
---
本番環境は、それ以外の環境とは異なり、OrBIT運用チームによるセキュリティエスカレーションのサービスレベルが上がります。
>セキュリティエスカレーションの詳細については以下を参照してください。
> - [セキュリティエスカレーション](/guide/aws/monitoring#セキュリティエスカレーション)

従って、本番環境の払い出しの際には、OrBIT運用チームがサービスの計画や体制を把握できるだけの以下のような情報の提出が必要になります。
...
例えば、BDR承認済みの基本計画書などを、本番環境の払い出し申請の際に添付してください。
...
| メンバー情報 |  利用するメンバーの情報です。 | 部門名<br>名前(英語表記)<br>Eメールアドレス<br>役割 ([こちら](/guide/aws/id-management#役割の一覧)から選んでください)<br>が必要になります。 |
...
| ログインURL | SSOポータルのログインURLです。 |　SSOポータルに関しては、[こちら](/guide/aws/id-management#ssoポータル)をご覧ください。 |
...
SSOポータルとは何かについては、以下を参照して下さい。
- [SSOポータルとは？](/guide/aws/id-management#ssoポータル)
...
::: tip NOTE
技術開発部 クラウド基盤チームには、AWSの知識が豊富な方が多数在籍していますので、質問や困りごとがありましたら、お気軽にご連絡ください。
:::

### 各種申請
---
プロジェクトアカウントの払い出し、情報更新および解約は、以下のフォームから申請することができます。
...
詳細については、以下を参照して下さい。
- [ベースラインについて](/guide/aws/baseline)

::: danger IMPORTANT
ベースラインには、利用しているサービスの伴う利用料金が発生します。この料金はプロジェクト側の利用料金として配賦されます。
...
一般的に、このようなアクセス権限管理は、以下の事項に従う事が推奨されます。
...
### ルートユーザーでの作業依頼
[ルートユーザーでしか実行できないタスク](#ルートユーザーでしか実行できないタスク)が必要になった際は、以下のフォームからOrBIT運用チームへ作業依頼を申請する事ができます。
...
::: tip INFO
例えば、`TestProject`というプロジェクトに払い出されたアカウントのルートユーザは、`testproject1@jvckenwood.com`というメールアドレスになります。また、同プロジェクトで別アカウントが必要になった際は、`testproject2@jvckenwood.com`のように連番で別メールアドレスが発行されます。
:::
...
尚、ルートユーザーのメールアドレスは[SSOポータル](/guide/aws/id-management#プロジェクトアカウントへのアクセス手段)から確認することができます。
...
::: tip INFO
*Organization*のRootでは、すべてのAWSアカウントの一括請求を行うことができます。
詳細については、[「一括請求による支払いと社内配賦」](/guide/aws/billing#一括請求による支払いと社内配賦)をご覧ください。
:::
