...
OrBITでは、上に挙げた考慮事項をはじめ、様々な困り事を解決する仕組みを提供しますので、
クラウドをより<font color="blue">**「簡単に」**</font><font color="blue">**「安心して」**</font><font color="blue">**「効率的に」**</font>利用することができます。
...
### 1. OrBITコア
OrBITコアは、**SoR**の側面における支援を提供し、クラウド利用に関して全社的なガバナンスを維持することを目的としています。

本システムは、プロジェクト側に払い出したクラウド環境にプロビジョニングされたOrBITベースラインというリソースを介して連携することで支援を実現しています。

#### OrBITベースライン
OrBITベースラインとは、OrBITから払い出されたすべてのクラウド環境に共通でプロビジョニングされるリソース群です。プロジェクト側のクラウド環境で収集した監査ログやセキュリティに関する情報を、OrBITコアへ転送する仕組みを備えます。

プロビジョニングされるリソースは利用するクラウドベンダ毎に異なりますので、詳細については以下を参照してください。
- [「AWSにおけるベースラインのリソース一覧」](/guide/aws/reference/baseline.html#リソース一覧)

### 2. OrBITナレッジベース
ナレッジとは、私たちが支援してきた様々な案件から得た知見や技術を、汎用的かつ簡単に利用することができるようにしたモノです。
> ナレッジの詳細に関しては、[「ナレッジとは？」](/knowledge/introduction.html)を参照してください。

OrBITナレッジベースは、ナレッジの検索手段や利用方法を提供し、**SoE**の側面において支援します。
プロジェクト側は必要に応じて、セルフサービスでナレッジを利用することができます。
...
- **ナレッジ検索機能**
    ...
    OrBITナレッジベースへアクセスし、プロジェクトが所望するナレッジが提供されているかを検索することができ、該当するナレッジが見つかった場合には、使い方や仕様を確認することができます。また、OrBIT開発チームに対して、改善要望やフィードバックを送ることができます。
